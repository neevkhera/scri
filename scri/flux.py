...
@swsh_indices_to_matrix_indices
def p_plusminus(ell_min, ell_max, sign, s):
...
# and defined here.
@swsh_indices_to_matrix_indices
def eth_N_eth_chi_z_h(ell_min, ell_max, s=-2):
...
@swsh_indices_to_matrix_indices
def h_ethbar_chi_z_eth_N(ell_min, ell_max, s=-2):
...
@swsh_indices_to_matrix_indices
def eth_N_eth_chi_h_plusminus(ell_min, ell_max, sign, s=-2):
   ...
   <\eth N|\eth \chi|h> = \sqrt{\frac{(2*l1+1)(2*l2+1)}{4\pi*(2*l3+1})}*\sqrt{2}*<l1,-1,l2,-s|l3,-1-s>*<l1,m1,l2,m2|l3,m3>
    ...
    We use `mat_el_eth_N_eth_chi_h` to compute the matrix elements.
    ...
    def mat_el_eth_N_eth_chi_h(s, l3, m3, l1, m1, l2, m2):
    ...
    for ell in range(ell_min, ell_max + 1):
        ...
        for ellp in range(ellp_min, ellp_max + 1):
            for m in range(-ell, ell + 1):
                ...
                if (mp < -ellp) or (mp > ellp):
                    continue
                yield (ellp, mp, ell, m, (prefac * mat_el_eth_N_eth_chi_h(s, ellp, mp, 1.0, sign, ell, m)))


eth_N_eth_chi_h_plus = functools.partial(eth_N_eth_chi_h_plusminus, sign=+1)
eth_N_eth_chi_h_minus = functools.partial(eth_N_eth_chi_h_plusminus, sign=-1)
eth_N_eth_chi_h_plus.__doc__ = eth_N_eth_chi_h_plusminus.__doc__
eth_N_eth_chi_h_minus.__doc__ = eth_N_eth_chi_h_plusminus.__doc__


@swsh_indices_to_matrix_indices
def h_ethbar_chi_eth_N_plusminus(ell_min, ell_max, sign, s=-2):
    ...
    We use `h_ethbar_chi_eth_N_plusminus` to compute the matrix elements.
    ...
    def mat_el_h_ethbar_chi_eth_N(s, l3, m3, l1, m1, l2, m2):
    ...
    for ell in range(ell_min, ell_max + 1):
        ...
        for ellp in range(ellp_min, ellp_max + 1):
            for m in range(-ell, ell + 1):
                ...
                if (mp < -ellp) or (mp > ellp):
                    continue
                yield (ellp, mp, ell, m, (prefac * mat_el_h_ethbar_chi_eth_N(s, ellp, mp, 1.0, sign, ell, m)))


h_ethbar_chi_eth_N_plus = functools.partial(h_ethbar_chi_eth_N_plusminus, sign=+1)
h_ethbar_chi_eth_N_minus = functools.partial(h_ethbar_chi_eth_N_plusminus, sign=-1)
h_ethbar_chi_eth_N_plus.__doc__ = h_ethbar_chi_eth_N_plusminus.__doc__
h_ethbar_chi_eth_N_minus.__doc__ = h_ethbar_chi_eth_N_plusminus.__doc__
...
def boost_flux(h, hdot=None):
    ...
    _, eth_hdot_eth_chiplus_h = matrix_expectation_value(eth_hdot, functools.partial(eth_N_eth_chi_h_plus, s=-2), h)
    _, eth_hdot_eth_chiminus_h = matrix_expectation_value(eth_hdot, functools.partial(eth_N_eth_chi_h_minus, s=-2), h)
    _, h_eth_chiplus_eth_hdot = matrix_expectation_value(h, functools.partial(h_ethbar_chi_eth_N_plus, s=-2), eth_hdot)
    _, h_eth_chiminus_eth_hdot = matrix_expectation_value(h, functools.partial(h_ethbar_chi_eth_N_minus, s=-2), eth_hdot)
    ...
    _, eth_hdot_eth_chiz_h = matrix_expectation_value(eth_hdot, functools.partial(eth_N_eth_chi_z_h, s=-2), h)
    _, h_ethbar_chiz_eth_hdot = matrix_expectation_value(h, functools.partial(h_ethbar_chi_z_eth_N, s=-2), eth_hdot)
    ...
    boost_flux[:, 0] = (0.5) * ((1 / 8)* ((ethbar_hdot_chiplus_ethbar_h + ethbar_hdot_chiminus_ethbar_h)
            - (eth_hdot_chiplus_eth_h + eth_hdot_chiminus_eth_h)
            + 6 * (hdot_chiplus_h + hdot_chiminus_h)
            + (ethbar_h_chiplus_ethbar_hdot + ethbar_h_chiminus_ethbar_hdot)
            - (eth_h_chiplus_eth_hdot + eth_h_chiminus_eth_hdot)
            + 6 * (h_chiplus_hdot + h_chiminus_hdot))
        - (1 / 2) * np.multiply(h.t, (hdot_chiplus_hdot + hdot_chiminus_hdot))
        + (-1 / 4) * (eth_hdot_eth_chiplus_h + eth_hdot_eth_chiminus_h)
        + (1 / 4) * (h_eth_chiplus_eth_hdot + h_eth_chiminus_eth_hdot)
    ).real

    # This is the component in the y direction. y = 0.5 * ( plus - minus).imag
    boost_flux[:, 1] = (0.5) * ((1 / 8) * ((ethbar_hdot_chiplus_ethbar_h - ethbar_hdot_chiminus_ethbar_h)
            - (eth_hdot_chiplus_eth_h - eth_hdot_chiminus_eth_h)
            + 6 * (hdot_chiplus_h - hdot_chiminus_h)
            + (ethbar_h_chiplus_ethbar_hdot - ethbar_h_chiminus_ethbar_hdot)
            - (eth_h_chiplus_eth_hdot - eth_h_chiminus_eth_hdot)
